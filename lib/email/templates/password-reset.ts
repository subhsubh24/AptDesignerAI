 ...
 * @param resetUrl absolute https link to our own /reset-password page carrying a
 *   `token_hash`, which that page redeems with `verifyOtp`. Deliberately NOT
 *   Supabase's `action_link`: `createBrowserClient` hardcodes `flowType: "pkce"`
 *   after spreading caller options, so an implicit hash-fragment callback would
 *   throw AuthPKCEGrantCodeExchangeError on every otherwise-valid link.
 */
