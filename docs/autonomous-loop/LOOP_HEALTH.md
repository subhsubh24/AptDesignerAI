...
LOOP_HEALTH:
  ...
  this_run:
    ...
    ci_flake: "NOT flake — a REAL regression this run introduced, caught by the required journeys gate. F7's screenshot capture settled with waitForLoadState(\"networkidle\"), whose own default timeout (30s) equals Playwright's per-test timeout; on the authed tier it did not settle, so two captures per step ate the whole budget and timed out 'onboarding starts without error' on run AND retry; invisible on the public tier, the only tier runnable locally. MECHANISM NOT ESTABLISHED: a reviewer falsified my stated cause (/api/billing/status is a one-shot fetch in app/saved/page.tsx:73, not a dashboard poll) — the fix is proven empirically (24/24 in 40.2s vs 1 failed + 1 flaky), the explanation is not. CORRECTION worth recording: mid-run I read a stale in_progress status, concluded from #733 also looking slow that the latency was infrastructure, and wrote that into this field — the job had in fact FAILED in 5 minutes. #733 passed; the failure was ours alone. Waits inside a capture are now bounded (1.5s settle / 1s fonts / 10s shot). SECOND CORRECTION: I claimed the committed PNGs were byte-identical before and after — false, and reviewer-caught; 9 of 10 match, public-pricing-mobile.png differs by 7 bytes (sub-pixel antialiasing). I hashed one file and generalised to ten. Residual, filed as issue #736: worst-case capture is ~25s inside a 30s per-test budget, two BARE unbounded waitForLoadState(networkidle) calls remain live at journeys.spec.ts:250 and :339 (pre-existing, authed tier, same primitive), and setViewportSize takes no timeout so hard-bounded overclaims."
  rolling_7d:
