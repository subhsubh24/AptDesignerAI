...
## Run 2026-07-18 (Run 97) — 5 file-disjoint value-bar changes: Stripe cancel-on-delete (D) + mobile paywall honest-trial disclosure (D/B) + TV-rule coverage (F2) + gallery functional filters (A) + RC identity-sync logging (B). ALL 5 both-Sonnet-APPROVED (3 needed a 2nd review cycle; every REQUEST_CHANGES was a REAL gap, fixed + re-approved).

### State on entry
- Cold container. Working branch `claude/sleepy-goldberg-4i1hhw` at origin default tip `28b708a` (Run 96 #653), 0/0 divergence vs `claude/ai-apartment-design-app-iHAdb`. `npm install` root. Baseline gate GREEN (tsc, 2161 tests, determinism). DEEP AUDIT NOT due (ran Run 95; next ~Run 99) → ran an 8-Haiku-scout sweep.

### Scout sweep (8 Haiku lenses, read-only)
- **security/RLS: CLEAN** — all migrations through 029 ENABLE RLS (tenant→auth.uid(), shared/internal→RLS+no-policy); all getAdminClient sites legitimately shared/internal & gated; all write/LLM/auth routes rate-limited; no IDOR/secret leak. Next migration 030.
- **web perf: no-op** — the 4 candidates (cron activation-emails, search/stream, search route recommendationMockups, diagnosis GET) were all single-DB-round-trip parallelizations = borderline micro-opts per the calibration anchor; batching 4 = padding → DROPPED (did not ship).
- **AI pipeline: 2 findings both DROPPED (correctly)** — (a) mockup-agent.ts:505-519 conditional `thinkingConfig` flagged "critical violation" is a FALSE POSITIVE: the code comment documents the Pro image model (gemini-3-pro-image) has "thinking always on", so omitting thinkingConfig for it is INTENTIONAL; forcing `{thinkingLevel:"low"}` onto a Pro image model would HARM it, and the QUALITY_SCORECARD explicitly states no violation exists today. (b) room-architecture-extractor.ts:159 hardcoded "low" thinking → raising to HIGH is a COST REGRESSION, not a win (cheapest-by-default contract); no evidence "low" produces bad extractions. Both left untouched.
- **business-case conversion lever: NOT built** — scout ranked a 7-day Pro trial (#1), Apartment add-on tier (#2), Studio/team tier as the buildable ARR levers. All are large multi-file billing features (entitlements + Stripe/RC + migration + cron + mobile paywall) with SPECULATIVE adoption-% lift — exactly what a readiness auditor rejects if baked into the case to clear the $100K floor — and human-gated on Stripe/RC SKU config. Deferred as NAMED follow-ups; not a rushed multi-change run.

### Shipped — 5 file-disjoint value-bar changes (all both-Sonnet-APPROVED)
- **(1) COMPLIANCE/D** `lib/billing/stripe.ts` (+`cancelSubscription`, `isStripeConfigured`) + `app/api/user/delete/route.ts` + `__tests__/api/user-delete.test.ts` — cancel a live Stripe subscription BEFORE the auth-delete cascades the stripe_customers mapping away (a deleted account was still being billed — Apple 5.1.1(v) + consumer harm). Guarded on stored status (active/past_due/unpaid); no-op pre-launch or already-cancelled; 502-without-delete on cancel failure. **2nd cycle:** Reviewer A caught the billing-lookup discarded its query `error` (a lookup failure would silently proceed to delete + orphan the sub) → now fails CLOSED (502) on lookup error too + regression test (+6 tests).
- **(2) COMPLIANCE/D+B** `mobile/src/components/paywall-sheet.tsx` — paywall hardcoded free-trial copy (sublines/CTA/legal/a11y) with NO RC trial configured (Apple 3.1.2/FTC false disclosure). Now derives trial text from the real product intro offer (`introPrice.price === 0`, period-unit mapping), shows it ONLY when a trial genuinely exists, honest auto-renewal copy otherwise; + restore-button loading text + accessibilityState. **2nd cycle:** Reviewer B caught the restore label/busy keyed off the SHARED `purchasing` flag (would read "Restoring…" during a normal purchase) → split into a `restoring` flag; `busy = purchasing || restoring` still mutually disables both.
- **(3) TEST/F2** `__tests__/validation/ergonomics.test.ts` — cover the previously-untested `tvViewingRule` (1.5–2.5× diagonal): pass-via-hint / too-close / too-far / 120" fallback / not-applicable-no-seating. **2nd cycle:** Reviewer A caught no boundary-adjacent value pinned the exact 1.5/2.5 constants or `>=`/`<=` inclusivity → added a 60" TV boundary-inclusive test (90"/150" pass) + a constants-pinning test (88.8" too-close, 151.2" too-far — inside a drifted 1.3/2.7 band). Live-mutation-verified: a coefficient drift AND an inclusivity flip each fail the new tests. Test-only, no prod code touched.
- **(4) DESIGN-BAR/A** `app/gallery/page.tsx` + new `app/gallery/gallery-showcase.tsx` — the style "tabs" looked interactive (one rendered selected) but were non-interactive divs filtering nothing (false-affordance dead-UI tell). Extracted a client component with real filter state — focusable `<button>`s (aria-pressed + focus rings) that filter examples by style; page stays a server component (keeps metadata); example data moved verbatim; empty-state handled.
- **(5) RELIABILITY/B** `mobile/src/app/_layout.tsx` — `Purchases.logIn/logOut` errors were swallowed with `.catch(() => {})`; a failure desyncs the RC app-user id from Supabase (purchase misattribution) with no breadcrumb → log via console.warn (server-side entitlement checks remain the safety net; observability only).

### Merge outcome + gate
- Single branch per this run's git constraint → pushed to `claude/sleepy-goldberg-4i1hhw`. Final merged-tree gate GREEN: tsc clean, **2174 tests** (2161 +13), determinism, `eslint .` 0 (touched), mobile `tsc --noEmit` + eslint clean.
- **No ROADMAP box ticked** — all 5 are partial-surface compliance/coverage/design hardening within already-checked or standing tracks (D1 already [x]; F2 standing; Track A/B ongoing).
- **No migrations, no secrets, no new PENDING_OPS** — the Stripe cancel path is inert until STRIPE_SECRET_KEY is set (same already-tracked human-gated key).

### Lessons learned
1. **A "success" side-effect fix is only complete if the LOOKUP that decides whether to act also fails closed.** Cancel-on-delete discarded the stripe_customers query `error`; a transient read failure would silently proceed to delete and orphan a charging subscription — reintroducing the exact harm the change targets. Check the read's error the same way the write's is checked.
2. **A per-action loading label must key off a per-action flag, never a shared busy boolean** — else it lies about which action is in flight (a restore button reading "Restoring…" during a purchase — ironic on a change whose whole point is "don't show false text").
3. **Boundary-adjacent test values are required to pin magic constants + comparison inclusivity.** Coarse in-band/out-of-band values leave a coefficient drift (1.5→1.3) and an inclusivity flip (`>=`→`>`) both uncaught; a value exactly at the edge + a value just inside a plausibly-drifted band close it.
4. **Dropping scout findings is as much the job as shipping them.** This run correctly did NOT ship: 4 single-round-trip perf micro-opts (padding), 2 AI-pipeline findings (a cost regression + a Pro-image-branch false-positive the scorecard confirms clean), and 3 speculative multi-file billing "conversion levers" (adoption-% lift is readiness-auditor-rejectable + human-gated on SKU config).

### Rotation guide for next run
- **DEEP AUDIT ran Run 95 (2026-07-17) — next due ~Run 99** (or >24h/~4 runs).
- **Ship blockers UNCHANGED & not headlessly buildable:** functional_reality (DATA_BACKEND cutover — PENDING_OPS); design_taste (authed-axe + F7 committed screenshots); business_case_strength (without-annual ARR ~$99.9K < $100K floor — human-gated migration 021 + ANNUAL_BILLING_ENABLED=true, OR a real conversion-lift FEATURE).
- **Named follow-ups (buildable, NOT built):** (a) mobile account-deletion (`app/api/mobile/account/route.ts`) can't cancel App Store/Play IAP server-side (stores own the sub) — the honest compliance is client-side "cancel via the App Store/Play" messaging on the mobile delete flow (a mobile-UI copy change, buildable next run). (b) The 3 business-case conversion levers (7-day trial / Apartment add-on / Studio tier) remain buildable but are large + adoption-%-speculative + SKU-config-gated — only build with an honest, non-floor-gamed model. (c) [carry] annual-billing "Cancel anytime." string in pricing (gated behind isAnnualBillingEnabled()).
- **DO-NOT-RE-FLAG (carry prior lists +):** Stripe cancel-on-delete (SHIPPED — web route, fail-closed on lookup + cancel error); mobile paywall free-trial copy (FIXED — driven by real introPrice, honest fallback); tvViewingRule coverage (COVERED — incl. boundary/constants); gallery decorative tabs (SHIPPED — real accessible filters); RC logIn/logOut swallow (FIXED — console.warn); mockup-agent conditional thinkingConfig (INTENTIONAL Pro-image branch, scorecard-confirmed clean — NOT a violation); room-architecture-extractor "low" thinking (INTENTIONAL cheapest-default — raising is a cost regression); web-perf single-round-trip parallelizations (borderline micro-opts, not standalone-worthy); + all prior carried non-issues (deepseek `.at(-1)` fixed; cron double-send fixed; lookupColorAsync covered; pricing/privacy/account compliance shipped; global-error CSS-free boundary; topbar step-indicator + emerald; npm-audit --force canary; mockups IDOR fixed; diagnosis project?. graceful; lookupDurability prefix-strip; web.ts invalid-date dead; soft save-count race; mobile hardcoded LED/splash; LABELED_DIMENSION_REGEX inches dead; bundle-math coffee-table band + spatial-math undersized guard intentional; webhook fire-and-forget intentional; waitlist confirm UPDATE-before-SEND low-value).
