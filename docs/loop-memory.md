...
### State on entry
- On the designated dev branch `claude/sleepy-goldberg-x5brvk` (tip `d22dc6d` / #630). NOTE on branch model: the session's explicit Git Development Branch Requirements pin work to `claude/sleepy-goldberg-x5brvk`; that branch is AHEAD of the nominal git-default `claude/ai-apartment-design-app-iHAdb` (carries #626–#630 + Run 88). No open PRs on entry. Baseline gate GREEN after cold `npm ci`: tsc clean, **2077 tests** pass / 11 skip, determinism green, eslint clean.
- No DEEP AUDIT due (ran Run 88; next ~Run 92). Ran a 4-Haiku-scout sweep + consumed QUALITY_SCORECARD + GROWTH_STATUS as DATA.

### Scout sweep (4 Haiku) + what was rejected
Lenses: security/RLS, web-reliability/correctness, conversion+mobile, tests/coverage.
- **Security CLEAN** — independent sweep: RLS on all tenant tables incl. the newest migrations, all id-routes bound via userOwnsRoom/userOwnsProject/eq(user_id), SECURITY DEFINER hardened (empty search_path), no hardcoded secret. Mockups IDOR (#612) confirmed fixed.
- **Verified already-resolved (NOT re-shipped):** #348 entitlement fail-open — `lib/entitlements/server.ts:42-52` ALREADY fails CLOSED in production (grants only in dev), so the issue's "fails open in prod" premise is stale. #600 business-case annual disclosure — fixed by #601 (df9fc3c) already in the tree.
- **Rejected as CHURN — 5 correctness-scout "missing-tiebreaker" determinism sorts** (domain-router `prioritizeDomains`, orchestrator domain-summary LOG, saturation-math category sort, infer-preferences freq sort, color-math dominantRel). KEY REASONING: a missing sort tiebreaker is only an ACTIVE determinism bug when the sort's INPUT order is non-deterministic. JS `Array.sort` is stable (ES2019+) and Map/Object iteration is insertion-ordered, so all 5 — built by sequential in-memory iteration (`Object.entries`, `Array.from(map.values())`) — already produce reproducible output; ties resolve to a deterministic insertion order. Adding a tiebreaker would be behavior-neutral. This is DISTINCT from the real bugs prior runs fixed (#622 etc.): those were DB-row `created_at` sorts where Postgres row order isn't guaranteed without ORDER BY — a genuine non-determinism source. `check:determinism` is green; none of the 5 fails it. Shipping them = padding.
- **Free-tier "one room" copy fix — DROPPED.** The scout claimed dashboard "Free forever on one room" is misleading vs "unlimited analyses / 3 saved designs". But the free grant is genuinely ambiguous: `app/pricing/page.tsx` markets free as "1 room diagnosis" (implying a 1-room limit), while `FREE_SAVE_LIMIT=3` is a separate SAVE gate. A copy change risked being LESS accurate, not more. This is an owner-coherence question (like #502), not a clear headless fix. Anti-invented-claims + when-in-doubt-stop → dropped.
- **budget-allocation focalOnly test — DROPPED.** The scout proposed testing the `!target.focalOnly` over-spend suppression branch, but NO budget target sets `focalOnly: true` (grep: only the field decl + the guard). The branch is vacuous (focalOnly always undefined), so a test wouldn't kill a meaningful mutation. Verify a flag is actually set before testing its guard.

### Shipped — 3 file-disjoint value-bar changes (all both-Sonnet-APPROVED)
- **(1) CORRECTNESS/Track F** — `lib/validation/spatial-math.ts`: `parseDimensions` mis-parsed per-token unit marks. The positional `DIMENSION_REGEX` carried the unit only at the END of the whole match, so `6' x 4'` / `72" x 36"` / `72in x 36in` had the unit consumed right after the first value and the regex never reached the `x` separator → collapsed the spec into a bogus SQUARE (a `6' x 4'` rug → 72×72 instead of 72×48), corrupting `room_coverage_ratio` for any feet/inch-marked spec (rugs especially). Rewrote the regex to capture a per-token unit for each of up to 3 values, with an inherited-unit fallback (`[...tokens].reverse().find(t=>t.unit)`) so the trailing-unit form `6x4 ft` → 72×48 still holds; `''` (inch) precedes `'` (foot) in the alternation so `6''` reads as inches. +5 tests (foot marks, inch marks incl. `''`, spelled per-token, mixed units, non-adjacent inheritance `6ft x 4 x 8`). This closes the Run-88 rotation-guide follow-up (the exact `6' x 4'` foot-mark bug).
- **(2) TEST/Track F** — `__tests__/validation/ergonomics.test.ts`: 3 mutation-killing cases for `tvViewingRule` (TV viewing distance 1.5-2.5× diagonal), whose too-close/too-far/in-range branches had ZERO coverage. 65" TV → minGood 97.5", maxGood 162.5"; distance read from sofa placement ("7 feet from"=84"→too close, "14 feet away"=168"→too far, "10 feet from"=120"→passes).
- **(3) TEST/Track F** — `__tests__/validation/spatial-graph.test.ts`: 4 tests. The sight-line dimension (`sight_line_score`, 20% of the spatial overall) had ZERO coverage — added a tall item in center → focal-wall block, a tall item on the window wall → natural-light block, and a tall PLANT on the window wall EXCLUDED (isolates the `!n.category.includes("plant")` carve-out: the plant must be tall AND window-anchored so only the exclusion spares it). Plus one north+east cardinal-corner pair that passes `isAdjacentZone` ONLY via the `cardinalPairs` branch (distinct from the already-tested shared-zone shortcut).

### Reviews + outcome
- **6 reviews (3×2 Sonnet), all APPROVE.** Reviewers independently MUTATION-TESTED against the live source: change 1 — verified the `''`-before-`'` ordering is load-bearing (reordering fails the `6'' x 4''` test) and no ReDoS; Reviewer A flagged that the inherited-unit REVERSE-SCAN wasn't exercised by a genuinely non-adjacent case → added the `6ft x 4 x 8` test before commit (unit on the first token only, inherited by the later two). Change 2 — confirmed `&&`→`||` and `<`/`>` operator mutations each fail; no vacuous pass (a non-firing rule yields `undefined`, not `false`). Change 3 — empirically deleted the plant carve-out and flipped `isAdjacentZone`'s return, confirming each fails ONLY the matching new test.
- Integrated gate GREEN: tsc clean, **2089 tests** (2077 +12: 5 spatial-math + 3 ergonomics + 4 spatial-graph), determinism green, eslint clean on touched files. Committed to `claude/sleepy-goldberg-x5brvk` as 3 code commits (78e0b73 / d37e34f / 2963242) + this bookkeeping commit; pushed.
- **No ROADMAP box ticked** — all 3 are validation correctness/coverage hardening; no full Track item completed. **No migrations, no secrets, no new PENDING_OPS.**

### Lessons learned
1. **A "missing sort tiebreaker" is only a REAL determinism bug when the INPUT order is non-deterministic.** JS `Array.sort` is stable + Map/Object iteration is insertion-ordered, so in-memory sequentially-built inputs already produce reproducible output — a tiebreaker there is behavior-neutral churn. The class worth fixing is DB-row / `Promise.all`-completion-order sorts. Rejected 5 such candidates this run; consuming `check:determinism` (green) as evidence backed the call.
2. **Verify a guard's flag is actually SET before testing the guard.** The `!target.focalOnly` branch is vacuous because no budget target sets `focalOnly:true` — removing the guard is behavior-neutral, so a test can't kill a mutation. A scout can propose a test on dead/vacuous code; a 30-second grep for the flag's assignment prevents it.
3. **Two consecutive runs found the same regex failure class** — Run 88 fixed the labeled `90"W x 38"D` parse; Run 89 fixed the per-token foot/inch-mark parse (`6' x 4'`). A dimension regex that carries the unit only at the end silently mis-parses any per-value unit mark. Recording the specific bug in the rotation guide (Run 88 flagged this exact `6' x 4'` case) paid off directly.

### Rotation guide for next run
- **DEEP AUDIT next due ~Run 92** (ran Run 88). Next couple of runs can lean on scouts/scorecard.
- **Ship blockers unchanged & not headlessly buildable:** functional_reality (in-memory→Supabase cutover, PENDING_OPS `cutover-to-persistent-data`), design_taste (authed-axe + F7 committed screenshots — sandbox-unrunnable), business_case_strength (without-annual ARR ~$99.9K < $100K floor — human-gated migration 021 + `ANNUAL_BILLING_ENABLED=true`, OR a REAL conversion-lift feature that honestly justifies >4% paywall/trial conversion — the latter is the biggest headlessly-buildable value lever left for a run with budget for a larger unit: paywall/onboarding/time-to-wow).
- **Test-coverage candidates surfaced but NOT taken this run (file-disjoint, real branches — for a future run):** harmony-math specificity/placement bonus + clamp (`lib/validation/harmony-math.ts:202-206`, light existing coverage); ergonomics `parseTvDiagonal` bounds (20-120") not directly asserted. NOT the budget focalOnly one (vacuous).
- **Larger open epics (unchanged):** G4 login lockout/backoff + reset/verification enumeration guards (needs a server-side `/api/auth/login` route); §34 pre-launch demo (#475); §11 media-gen adapter (#470); perf #385 pgvector RPC (post-cutover only); mobile setState hygiene (dev-only). Migrations 021/022-023/024/025/026/027/028/029 + DATA_BACKEND cutover remain human-gated in PENDING_OPS.
- **Do NOT re-attempt the 5 determinism-tiebreaker sorts** (behavior-neutral churn on deterministic inputs) or the free-tier copy change (owner-coherence question, not a headless fix) — both deliberately rejected this run with reasoning above.
