...
`captureJourneyStep` settled with `waitForLoadState("networkidle")`. Its default timeout is 30s — **the same as Playwright's per-test timeout**. On the public tier nothing polls, so it resolves instantly and the local run was green. On the AUTHED screens the dashboard polls `/api/billing/status`, so networkidle NEVER settles; two captures per step consumed the entire test budget and timed out `onboarding starts without error` on the run *and* its retry. `.catch()` was no defence — **a wait that is still PENDING never rejects**, which a reviewer had flagged as a theoretical edge and I accepted as low-risk.
...
Fixed by hard-bounding every wait inside a capture (1.5s settle / 1s fonts / 10s shot). **The committed PNGs are byte-identical before and after**, which proves the bound costs no evidence. **Rule: instrumentation added to a test must be bounded BELOW the test's own timeout, or it becomes the thing that fails the test.**
