...
## Run 2026-07-19 (Run 99) — DEEP AUDIT (8-lens, due) + 6 file-disjoint value-bar changes: 2 correctness/reliability fixes on paid paths (C1 bundles null-guard + C6 analyze-apartment array-guard) + 3 Track-F2 validation coverage tests + 1 Track-F5 config-doc. ALL 6 both-Sonnet-APPROVED, 0 re-review cycles.

### State on entry
- Cold container. Working branch `claude/sleepy-goldberg-5ajok0` at origin default tip `e93fe56` (#663 vendor design skill), 0/0 divergence vs `claude/ai-apartment-design-app-iHAdb`. `npm install` root (reverted the package-lock artifact). No /mobile source touched this run.
- Baseline gate GREEN: tsc clean, **2166 tests** / 11 skip, determinism, `eslint .` 0 ERRORS (19 warnings, ALL in the vendored `.agents/skills/impeccable/scripts/detector/detect-antipatterns-browser.js` — third-party skill source out of blast radius, pre-existing; app source clean).
- **DEEP AUDIT was DUE** (last ran Run 95; >4 runs / >24h) → ran the full 8-Haiku-lens holistic sweep BEFORE selecting.
- **Scorecard (DATA, as_of 2026-07-13):** overall C, ship_gate false; three ship_critical below A unchanged & human/CI-gated (functional_reality C — DATA_BACKEND cutover; design_taste B — authed-axe + F7 screenshots; business_case_strength B — without-annual ARR ~$99.9K < $100K floor). GROWTH pre_launch 0/null → no lever signal.

### DEEP AUDIT — 8 Haiku lenses (whole codebase, read-only)
- **correctness/dead-code:** 1 finding → C1 (bundles/evaluate null candidate_products). Otherwise mature/clean.
- **security/RLS: CLEAN** — all migrations ENABLE RLS through 029 (tenant→auth.uid(), shared/internal→RLS+no-policy); next migration 030; ~30 getAdminClient sites shared/internal & auth/sig/token-gated; SSRF validator + ownership guards + Stripe HMAC-verified idempotent webhook; no IDOR/secret leak.
- **performance: CLEAN** — hot paths already parallelized (analyze-apartment per-room Promise.all, saved-designs, products/evaluate, diagnosis, area-analysis); remaining sequential awaits are dependent chains or end-of-pipeline tails; payloads bounded (MAX_JSON_BYTES), embeddings projection-narrowed. No churn.
- **a11y/design-bar: CLEAN** — icon-button aria-labels, htmlFor labels, img alts, focus-visible rings, aria-live, Radix focus traps, Lucide (no emoji-as-UI), warm-editorial tokens consistent. No vibe-code.
- **monetization/store/mobile: CLEAN** — server-side hasProEntitlement(Web) gates, signature-verified idempotent webhook w/ normalizeBillingTier, fail-closed-on-misconfig/fail-open-on-outage, account-deletion (web+mobile), auto-renewal+pricing disclosures present, mobile cross-user photo-session leak already fixed (Run 97), side-effects awaited.
- **test/eval coverage:** 3 mutation-provable gaps → C3/C4/C5 (the `checks===0→0.9` neutral default in access-constraints/outlet-reach/code-compliance, each feeding harmony-math composite, each unasserted at HEAD).
- **dependency/config:** timeouts ALL CLEAR (Gemini 180s, DeepSeek 120s, Tavily 15s, Resend 10s, RevenueCat 5s, Turnstile 5s, all < maxDuration budget). 1 reliability finding → C6 (analyze-apartment as-any array persistence). 2 env-doc gaps → C7/C8.
- **artifact-freshness:** strong integrity (pricing $29/$49/$399 consistent code↔docs↔listing; all 12 processors real; privacy claims match code). 1 minor nit (BUSINESS_CASE Pro Annual reads live though gated OFF — disclosed twice LATER at lines 79-85/366) → **HELD below-bar** (editorial churn; info already present).

### Shipped — 6 file-disjoint value-bar changes (all both-Sonnet-APPROVED; 8 reviews, 0 re-review cycles)
- **(1) CORRECTNESS** `app/api/bundles/evaluate/route.ts` + new `__tests__/api/bundles-evaluate-null-products.test.ts` — `candidate_products` from the nested join can be null (deleted product / broken FK); it flowed unfiltered into `evaluateBundle` (`bundle` is `any` → tsc blind), whose first deref (`p.id` at bundle-optimizer:42, `p.category` at :229) throws an UNCAUGHT 500 on a paid path AFTER `createAgentRun` — so the bundle sticks at "pending" and the agent run orphans. Fix: `.filter((p): p is CandidateProduct => p != null)` + a 400 when no resolvable products remain (avoids burning a paid LLM call on an empty set). Both reviewers reverted the guard → both tests fail (null reaches evaluateBundle; empty→201-not-400). Mutation-proven.
- **(2) RELIABILITY/F4.1** `app/api/analyze-apartment/route.ts` + extended `__tests__/api/analyze-apartment-persistence.test.ts` — LLM `.add`/`.replace`/`.keep` are consumed as arrays at TWO sites. The test caught a SIBLING crash the audit missed: the synthesis step at route.ts:320 does `(a.replace||[]).join()` → a non-array object (`{note:"swap"}`) is truthy so `||[]` keeps it → `.join` throws → 500 on the CORE path. The audit-named site (persistence :392-394) stored a non-array verbatim into `room_diagnoses.action_list` (JSONB) → downstream `for (const item of action_list)` (infer-preferences:202, `?? []` guards only null) + `action_list.map` (diagnosis-expansion:104) throw in a LATER request. Fix: module-scope `firstArray(...vals)` (first arg that is `Array.isArray`, else []) at BOTH sites — behaviorally identical to `x||y||[]` for well-formed arrays (arrays always truthy), only differs for the malformed non-array. Both reviewers mutation-verified each site independently. (Comment corrected post-review: cited infer-preferences/diagnosis-expansion, not correction-planner whose `missing_categories` is a separate Zod-validated field.)
- **(3)(4)(5) F2** `__tests__/validation/{access-constraints,outlet-reach,code-compliance}.test.ts` — pinned each function's `checks===0 → 0.9` neutral-default score (exercised by existing tests but `.score` never asserted; each is a weighted input to harmony-math's composite — access 0.01, outlet 0.03, code 0.01). Realistic COMMON inputs: accents-only room (no LARGE_ITEM_CATEGORIES), no powered items, no building-research data (the last is the majority case). All mutation-proven (0.9→0.85 fails all three; distinct functions in distinct files → not padding).
- **(6) F5** `.env.example` — documented two prod-affecting env vars the code reads but the setup template omitted: `NEXT_PUBLIC_APP_URL` (Stripe post-checkout redirect target at checkout/portal/mobile-share routes, fallback `https://aptdesignerai.com`, + CORS allowlist origin — a staging deploy without it silently redirects users to PROD after a real charge) and `EMAIL_PHYSICAL_ADDRESS` (when unset, force-dry-runs ALL marketing-lifecycle sends even with RESEND_API_KEY set; transactional waitlist_confirm exempt). Both reviewers verified every claim against the code line-by-line.

### Merge outcome + gate
- Single branch per this run's git constraint → ONE PR to default (code + this bookkeeping). Merged-tree gate GREEN: tsc clean, **2172 tests** (2166 +6: 2 bundles + 1 analyze-apartment-coerce + 3 validation defaults), determinism, `eslint .` 0 (touched). No /mobile source touched. package-lock install artifact reverted.
- **No ROADMAP box ticked** — DEEP AUDIT/F5 is a recurring cadence not a per-run checkbox; F2 already [x]; C1/C6 are correctness/reliability hardening on paid paths, not full Track items (Track A/C already [x]). **No migrations, no secrets, no new PENDING_OPS** (NEXT_PUBLIC_APP_URL is deploy config; EMAIL_PHYSICAL_ADDRESS already tracked in PENDING_OPS as part of the Resend connect step).

### Lessons learned
1. **A route-test with a realistic malformed-LLM input can surface a SIBLING bug the read-only audit missed.** The C6 audit named the persistence line (:392); the test's non-array `.replace` object crashed the EARLIER synthesis `.join()` (:320) with a 500 first — so the true fix had to guard both consumer sites, not just the one the scout pointed at. Write the test to the realistic failure shape, then follow where it actually breaks.
2. **`x || y || []` is a latent non-array-persistence bug wherever `x` is untrusted LLM output.** A truthy non-array (string/object) is kept verbatim; `Array.isArray`-based `firstArray()` is behaviorally identical for well-formed arrays (arrays are always truthy) and only diverges on the malformed shape — the exact fix, zero collateral.
3. **A `bundle`/row typed `any` from an untyped Supabase `.select()` silently defeats tsc's null-safety** — the null `candidate_products` was invisible to the type checker. When a route maps nested-join relations, filter nulls explicitly; don't trust the type.
4. **A neutral-default score branch (`checks===0 → 0.9`) is worth pinning when it (a) fires on common inputs and (b) feeds a composite** — that's real branch behavior, not boundary padding; three distinct sibling dimensions each unasserted justified three tests (not one repeated with nudged boundaries).
5. **When a DEEP AUDIT is due, run it FIRST and let its findings BE the candidate pool** — this run's 6 changes all came straight from the audit; 4 of 8 lenses were CLEAN (a valid, valuable result), converging on the 2 real correctness fixes + 3 coverage + 1 config the audit actually found.

### Rotation guide for next run
- **DEEP AUDIT ran Run 99 (2026-07-19) — next due ~Run 103** (or >24h/~4 runs). Runs 100-102 can lean on scouts + scorecard.
- **Ship blockers UNCHANGED & not headlessly buildable:** functional_reality (DATA_BACKEND cutover — PENDING_OPS `cutover-to-persistent-data`); design_taste (authed-axe + F7 committed screenshots — needs seeded-LLM E2E + push-and-watch-CI); business_case_strength (without-annual ARR ~$99.9K < $100K floor — human-gated migration 021 + `ANNUAL_BILLING_ENABLED=true`, OR a real conversion-lift FEATURE).
- **DO-NOT-RE-FLAG (carry prior lists +):** bundles/evaluate null candidate_products (FIXED — filtered + empty-400); analyze-apartment LLM-array persistence at synthesis :320 + persistence :392 (FIXED — firstArray at both); access/outlet/code `checks===0→0.9` default score (COVERED); NEXT_PUBLIC_APP_URL + EMAIL_PHYSICAL_ADDRESS .env.example (DOCUMENTED); BUSINESS_CASE Pro-Annual "reads live though gated" (HELD below-bar — already disclosed at lines 79-85/366, editorial churn); + all prior carried non-issues (mockup-agent.ts:504 HIGH = intentional image modality; createAgentRun outside-try = framework-caught 500; global-error CSS-free boundary; topbar step-indicator + emerald/amber semantic colors; npm-audit --force canary Next.js; mockups IDOR fixed; diagnosis `project?.` graceful; `.single()`→`.maybeSingle()` on memory-store reads NON-ISSUE; lookupDurability prefix-strip not mutation-provable; web.ts invalid-date dead; soft save-count race intentional; mobile hardcoded LED/splash; LABELED_DIMENSION_REGEX inches dead; bundle-math coffee-table/wood-species band + spatial-math undersized guard intentional; webhook fire-and-forget intentional; waitlist confirm UPDATE-before-SEND low-value; mobile paywall packagesToOptions else-branch — only if RC catalog grows a 3rd package type; cron double-send FIXED Run 96; mobile AsyncStorage quota-divergence untested-client edge).
