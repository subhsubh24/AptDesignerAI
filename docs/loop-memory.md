...
## Run 2026-07-25 (Run 114) — DEEP AUDIT (8-lens, due by the ~24h rule) + 5 file-disjoint value-bar changes. Three needed a REAL second review cycle; two of those were release-blocking "builds but does not work" defects caught by reviewers, not by me. Single designated branch → ONE PR.

### State on entry
- Cold container. Designated branch `claude/sleepy-goldberg-4xn83t` == default tip `8c1f784`, 0/0 divergence. Baseline GREEN: web tsc, **2276 tests** / 11 skip, determinism, mobile tsc.
- **DEEP AUDIT WAS DUE and the rotation guide said otherwise.** Run 113's guide said "next due ~Run 115"; the actual clock said Run 111's audit ran 2026-07-24 08:49 UTC and this run started 2026-07-25 16:12 UTC — **31h**, past the ~24h rule. Ran it. **Lesson: the ~24h clock wins over the run-number estimate; check the commit timestamp of the last audit, not the predicted run number.**

### DEEP AUDIT — 2026-07-25 (Run 114), 8 Haiku read-only lenses over the WHOLE codebase
- **correctness/dead-code: CLEAN.** No stub/TODO on a critical path, no fake-success, no unbounded external call, no `.optional()` env a critical path needs.
- **a11y/design-bar: CLEAN.** No axe-serious violation; the `e.target === e.currentTarget` guard is in place on the products Card.
- **store-readiness: CLEAN.** Account deletion, restore-purchases, subscription disclosure, permission strings, privacy manifest all verified against current Apple/Play rules. Remaining items are all owner infra (RC keys, EAS id, AASA, store forms).
- **security/RLS: CLEAN except ONE** — `getCurrentUserId()` handed back the fixed dev UUID on a signed-out request AND on an `auth.getUser()` throw, in production too → shipped (C3).
- **perf:** three ~50-100ms single-round-trip parallelizations (saved-designs POST, diagnosis GET, refine-chat POST). **ALL DROPPED** — the standing "sub-200ms micro-opt, standalone only, never batch" tier; batching three is padding.
- **artifact-freshness:** one open item — the "1 full room analysis" free-tier claim (BUSINESS_CASE tier table, pricing FAQ) vs the actual enforcement, which is a 3-SAVE limit with unlimited analyses. **DEFERRED as an owner decision, not a bug** (see below).
- **tests/evals/deps:** eval gold set is thin (2 fixtures) — real, deferred (needs a licensed real room photo + a REVIEWED-correct expected output; capturing pipeline output would be circular). Found the production-path npm advisories → shipped (C5).
- **mobile side-effect integrity: CLEAN except the known G4 leak** → shipped (C2).

### Shipped — 5 file-disjoint (all reached 2/2-Sonnet-APPROVE)
1. **feat(auth/G4) password reset — the flow did not exist at all.** `app/(auth)/forgot-password`, `app/(auth)/reset-password`, `app/api/auth/forgot-password`, login link, `lib/email` password_reset stage + template, 3 journey tests, 10 route tests. A user who forgot their password was **permanently locked out**: zero hits for `resetPasswordForEmail`/`forgot`/`reset` anywhere in the tree. Enumeration-safe (identical body for registered/unregistered), IP-limited 3/15min + Turnstile, and **honest when the email provider is not connected** — it returns `emailUnavailable` and routes the user to support rather than promising an inbox message nothing will deliver.
2. **fix(auth/G4) mobile auth enumeration** — `mobile/src/lib/auth/auth-errors.ts` + both screens + a parity test. Closes the leak Run 113 named as the open follow-up.
3. **fix(security) no fallback identity outside local dev** — `lib/supabase/server.ts` + the 5 saved-designs call sites now 401 on a null identity.
4. **fix(billing/store) paywall stops promising a trial it cannot confirm** — `mobile/src/lib/paywall-trial.ts` + paywall-sheet.
5. **chore(deps) non-force `npm audit fix`** — lockfile only; closes ws / protobufjs / @babel/core / vite.

### The three reviewer catches that mattered (maker ≠ checker earned its keep three times)
1. **The password-reset flow WOULD NOT HAVE WORKED — and every test I had written was green.** Reviewer A read the installed packages and found that `createBrowserClient` (@supabase/ssr) hardcodes `flowType: "pkce"` *after* spreading caller options, while `admin.generateLink` can only produce an **implicit** (hash-fragment) callback, because an admin-minted link has no PKCE verifier to pair with. auth-js's `_getSessionFromURL` throws `AuthPKCEGrantCodeExchangeError` the instant a pkce client meets an implicit callback — so **every valid reset link would have rendered "That link has expired."** Verified both facts myself before acting. Fix: stop relying on `detectSessionInUrl` entirely — the route mails `${origin}/reset-password?token_hash=<hashed_token>` and the page redeems it with `verifyOtp({token_hash, type:"recovery"})`, which POSTs straight to /verify with no flow-type branching. **LESSON: unit tests of a route prove the ROUTE; they say nothing about whether the client can consume what the route produced. When a feature spans a server mint and a client redeem, read the actual installed client's config — a green test suite is not evidence the two halves fit.**
2. **A timing oracle gave back exactly what the identical response body was hiding.** Only the registered-address branch paid for an outbound Resend round-trip before responding, so response latency separated "has an account" from "does not" no matter how identical the JSON was. Fixed with `waitUntil` (the primitive margin-meter already uses) so the send happens off the measurable clock and its failure is still logged. **LESSON: an enumeration-safety claim that only covers body + status is incomplete; ask what work each branch does before it answers.**
3. **Hiding the MESSAGE is not enough when the SCREEN differs.** On mobile signup, mapping "User already registered" to a neutral string still left the oracle intact: under auto-confirm a new address signs straight in while a taken one stops at an error box, and "an error box appeared at all" is the signal. Fixed by taking the same path web takes — attempt sign-in on the exists case, so both land in the app with the right password. **LESSON: for an enumeration fix, enumerate the OUTCOMES a user can observe (screen, navigation, latency), not just the strings.**
4. **A product-level flag is not a per-user entitlement.** The paywall's trial claim, even once derived from `introPrice`, would still over-promise to any returning iOS subscriber — intro offers are granted once per account, and `introPrice` is static product metadata. Now gated on `checkTrialOrIntroductoryPriceEligibility` on iOS (UNKNOWN/failed → no trial), while Android deliberately does not require it (Play filters ineligible offers server-side and that API always answers UNKNOWN there). **LESSON: when copy describes what a STORE will do, check the per-user API, not the catalogue.**

### Other lessons
5. **My own e2e assertions were wrong in a way that revealed a real fact about the app:** `getByRole("heading")` matched nothing, because `CardTitle` renders a styled `<div>` on **every** auth page. The pages work; the selector was wrong. (Not fixed here — `components/ui/card.tsx` is shared and out of this change's scope. Worth a standalone look: no auth page currently exposes a heading landmark.)
6. **Running the journey suite locally produced 8 failures, 5 of them pre-existing and environmental** — with no Supabase creds the mock client reports a session, so `/login` redirects and the login/signup/protected-route tests cannot pass here. I confirmed my 3 new tests pass on their own and folded the "Forgot password?" link assertion into the EXISTING login test rather than adding a test that could only fail for a reason unrelated to itself.
7. **`test-results/` is not in `.gitignore`.** Running Playwright leaves it in the working tree and it staged itself on a `git add -A`. Caught and removed before commit; the ignore entry belongs in the housekeeping PR.

### Merge outcome + gate
- Single designated branch → ONE PR. Merged-tree gate GREEN: web tsc, **2341 tests** (2276 +65), determinism, eslint 0 on touched files, mobile tsc + eslint, production build, and the 3 new journey tests RUN green.
- No migrations. No secrets. **No ROADMAP box ticked** — G4 stays `[ ]`: password-reset enumeration and the mobile leak are now closed, but login lockout/backoff still needs a server-side sign-in route, and self-serve reset stays inert until Resend is connected.

### Rotation guide for next run
- **DEEP AUDIT ran 2026-07-25 (Run 114) — next due ~2026-07-26.** Check the last audit commit's TIMESTAMP, not the predicted run number (this run's guide was wrong by a run).
- **NAMED buildable follow-ups (each standalone, do NOT batch-pad):** (a) G4's remaining half — login lockout/backoff, which needs a server-side sign-in route setting cookies via @supabase/ssr; a client-side counter is not security and must not ship as if it were. (b) `components/ui/card.tsx` CardTitle renders a `<div>` — no auth page has a heading landmark. (c) eval gold-set growth (needs a real licensed room photo + a reviewed-correct expected output; never capture pipeline output — that is circular).
- **#692 (share links sold as Pro but ungated) is still the live owner decision** — do NOT implement either option unilaterally. The free-tier "1 room analysis" claim vs the real 3-save limit is the SAME class and should go to the owner the same way, not be guessed at: the product currently gives away MORE than it advertises, so it is a revenue question, not a compliance one.
- **DO-NOT-RE-FLAG (carry ALL prior lists +):** the three sub-200ms perf parallelizations (saved-designs POST / diagnosis GET / refine-chat POST — micro-opt tier, never batch); password reset missing (SHIPPED); mobile raw `error.message` on login/signup (SHIPPED); `getCurrentUserId` mock-identity fallback (SHIPPED); paywall unconditional trial copy (SHIPPED); ws/protobufjs advisories (SHIPPED — remaining next/postcss/sharp/@vercel/analytics are the standing owner-gated major-bump item).
