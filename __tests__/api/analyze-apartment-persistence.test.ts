...
describe("analyze-apartment — per-room diagnosis persistence", () => {
  ...
  // route fans out one LLM call per room then persists SERIALLY — enough
  // rooms can push total latency past maxDuration, failing the whole run and
  ...
  it("caps analysis to the oldest 20 rooms when a project has more", async () => {
    ...
    const manyRooms = Array.from({ length: 25 }, (_, i) => ({
      ...
      name: `Room ${i}`,
      room_type: "bedroom",
      room_images: [{ image_url: `https://x/${i}.jpg` }],
    }));
    ...
    const diagnosesInserts: Array<{ room_id: string }> = [];
    mockCreateClient.mockResolvedValue({
      auth: { getUser: async () => ({ data: { user: { id: "owner-1" } } }) },
      from: (table: string) => {
        if (table === "projects") {
        ...
        if (table === "rooms") {
          return {
            select: () => ({ eq: () => ({ order: async () => ({ data: manyRooms, error: null }) }) }),
            update: () => ({ eq: async () => ({ error: null }) }),
          };
        }
        if (table === "room_diagnoses") {
    ...
    // Only the first 20 (oldest, by created_at order) were analyzed and persisted.
    expect(diagnosesInserts).toHaveLength(20);
