...
OWNER_ACTIONS:
  ...
  items:
    ...
    - id: verify-otp-expiry-vs-reset-quota
      ...
      priority: normal
      status: open
      why: "Run 127 added a per-RECIPIENT cap of 3 password-reset emails per 15 minutes (app/api/auth/forgot-password/route.ts, MAX_SENDS_PER_EMAIL / RATE_WINDOW_MS). Being over quota is deliberately silent — the caller still gets the neutral 200 — and that is only safe because a suppressed request means three recovery links were already ACCEPTED for that address inside the window, so the owner is holding a usable one. If the project's OTP expiry is ever configured at or BELOW 15 minutes, those earlier links can be dead by the time the owner asks, and the silent suppression becomes a real lockout for a flooded user. The codebase cannot read this setting, so it makes no claim about it — this entry is the check. (An earlier draft of the code comment asserted this was already tracked here; it was not, and a reviewer caught it.)"
      how: "Supabase Dashboard → Authentication → Providers → Email → 'Email OTP Expiration'. Confirm it is well above 15 minutes (the default is 1 hour, which is fine). If you have tightened it to 15 minutes or less, lower MAX_SENDS_PER_EMAIL in app/api/auth/forgot-password/route.ts so the quota window stays shorter than the link lifetime. Note Supabase's own security advisor flags LONG OTP expiry, so this is a genuine two-sided tradeoff, not a set-and-forget."
      blocks: none
