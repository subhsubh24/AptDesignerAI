...
OWNER_ACTIONS:
  ...
  items:
    ...
    - id: enforce-ci-required-checks
      ...
      followup: "STALE AS OF 2026-07-31 — the AUTHED tier IS green in CI now: run 30618992413 (PR #756) shows `run-journeys: running e2e/journeys.spec.ts (mode=full)` with E2E_AUTH_STACK=1 and 30 passed / E2E_JOURNEYS_PASSED=1, so sign-in does reach /dashboard against the supabase-local seed. What remains is promoting it to a REQUIRED check. enforce_admins stays false until Growth + Quality-Auditor routines also merge via --auto (today they don't, so admin-enforcement could strand their PRs)."
      blocks: none
    - id: wire-orphaned-e2e-specs-into-ci
      title: "Run e2e/public-pages.spec.ts + e2e/a11y.spec.ts in CI — today neither executes anywhere"
      priority: normal
      status: open
      why: "The `journeys` job's only test step is `bash scripts/run-journeys.sh`, and that script hardcodes SPEC=\"e2e/journeys.spec.ts\". So two committed spec files run in NO job. They rotted exactly as an unrun test does: Run 130 found three tests red — /support matching three headings at once, and /faq + /guides matching nothing at all because their expected text never matched those pages' h1s, wrong since the spec's first commit, so neither had ever passed once. Run 130 fixed all three and added a waitlist-CTA assertion, but until this is wired none of it GATES anything. e2e/a11y.spec.ts was worse than unrun: it FAILED on a real serious-impact color-contrast violation on /guides. UPDATE (Run 131, #761): that specific violation is FIXED — the /guides signup CTA was `bg-accent-warm text-accent-foreground` at 3.51:1 and now uses `--accent-warm-on-solid` at 5.11:1, with no `bg-accent-warm text-accent-foreground` left anywhere in app/ or components/. Stated precisely: the NAMED violation is gone; whether the spec passes as a whole is still unverified, because it continues to run in no job — which is exactly what this item is for."
      how: "In .github/workflows/ci.yml's `journeys` job, after `bash scripts/run-journeys.sh`, add a step running `npx playwright test e2e/public-pages.spec.ts e2e/a11y.spec.ts`. It needs the same placeholder Supabase identity the public tier uses (NEXT_PUBLIC_SUPABASE_URL=https://placeholder.supabase.co, NEXT_PUBLIC_SUPABASE_ANON_KEY=placeholder) or the middleware bypasses auth and serves the dashboard for every path. DO NOT add it as a required check until it is green — a11y.spec.ts is red today on the /guides contrast defect (tracked in docs/loop-memory.md as a named next-run item), so let the loop fix that first, THEN require it."
      blocks: none
    ...
    - id: ci-journeys-data-backend
      title: "Add DATA_BACKEND: \"supabase\" to the CI journeys job env so the authed suite exercises the PRODUCTION data path"
      priority: high
      status: open
      why: "The CI `journeys` job already stands up supabase-local and applies every migration, but its env block (.github/workflows/ci.yml) never sets DATA_BACKEND — so the authed journeys run against the IN-MEMORY store while a fully-migrated Postgres sits unused beside them. That is why QUALITY_SCORECARD.functional_reality has been held at C for six cycles: the green suite proves the memory backend, not the shipping config, and the 26 RLS policies still never execute at runtime. Run 122 investigated whether the loop could close this itself and established it CANNOT, for two independent reasons worth recording so no future run re-derives them: (1) the loop is barred from editing .github/ (a sensitive-file permission prompt hangs a headless run); (2) the obvious workaround — exporting DATA_BACKEND from scripts/run-journeys.sh, which IS loop-editable — does not work, because the CI job starts the app with `npm run start &` BEFORE invoking that script, so the env would never reach the already-running server process. There is also no `supabase` CLI and no supabase/config.toml in the loop's container, so this cannot be proven locally either."
      how: "In .github/workflows/ci.yml, add `DATA_BACKEND: \"supabase\"` to the `journeys` job's top-level `env:` block (alongside E2E_AUTH_STACK). The Supabase URL/anon/service-role vars are already exported into GITHUB_ENV by the existing 'Export local Supabase keys' step, so the fail-loud credential check in lib/supabase/server.ts will be satisfied. VERIFY: the journeys job stays green, and a seeded row written through the app's API is readable back through a user-scoped client (i.e. the RLS policies actually ran). EXPECT the first run to surface real defects — this path has never executed in CI. Migration 030 must already be applied before any PRODUCTION cutover (see cutover-to-persistent-data), but supabase-local applies every migration from scratch, so CI is safe to flip first and is the cheapest place to learn what breaks."
    ...
    - id: rate-limit-redis
      ...
      how: "Install the Upstash Redis Vercel integration (1-click from Vercel dashboard → Integrations), set UPSTASH_REDIS_REST_URL + UPSTASH_REDIS_REST_TOKEN env vars, swap lib/utils/rate-limiter.ts AND lib/utils/spend-limiter.ts to use @upstash/ratelimit (Sliding Window) / a shared Redis counter. ALSO (added Run 127, flagged by two reviewers): app/api/auth/forgot-password/route.ts keeps TWO of its own local in-memory Maps that this entry previously did not name — `ipBucket` (per-IP) and `emailBucket` (per-recipient mint cooldown; was a 3-per-window quota as of Run 127, changed to a 1-per-window cooldown in Run 149/#712 — see `verify-otp-expiry-vs-reset-quota` below for why). An owner following the two module names above verbatim would not touch them. The per-recipient cooldown is the one that matters most here: it exists specifically to stop a DISTRIBUTED flood at one address, and per-instance memory is exactly what a distributed attacker defeats (real ceiling = 1 x warm instances, down from 3 after Run 149's fix). Neither local Map has the periodic sweep lib/utils/rate-limiter.ts has, so both also grow unbounded under an address-cycling flood."
      blocks: rate-limiting-at-scale
    - id: verify-otp-expiry-vs-reset-quota
      title: "Verify the Supabase Auth email-link (OTP) expiry is comfortably longer than 15 minutes"
      priority: normal
      status: open
      why: "Run 127 added a per-RECIPIENT cap of 3 password-reset mint+send cycles per 15 minutes. Run 149 (#712) changed this to a 1-mint-per-15-minutes COOLDOWN (`RESET_COOLDOWN_MAX_MINTS` / `RATE_WINDOW_MS` in app/api/auth/forgot-password/route.ts) — the 3-mint version let 2-3 rapid requests each re-mint and overwrite the single `recovery_token` Supabase/GoTrue stores per user, silently invalidating whatever link the FIRST request had just mailed. Being on cooldown is deliberately silent — the caller still gets the neutral 200 — and that is only safe because a suppressed request means a recovery link for that address WAS accepted by the provider inside the window (once, not necessarily three times as this entry previously said), so the owner is holding a usable one — UNLESS that one attempt failed to deliver, in which case Run 149 ALSO changed the route to no longer release the cooldown slot on failure (closing a timing oracle the release used to create), so a real, non-attacker-triggerable delivery failure now also costs the rest of the window with no retry. If the project's OTP expiry is ever configured at or BELOW 15 minutes, that one link can be dead by the time the owner asks, and the cooldown becomes a real (if rare) lockout. The codebase cannot read this setting, so it makes no claim about it — this entry is the check. (An earlier draft of the code comment asserted this was already tracked here; it was not, and a reviewer caught it.)"
      how: "Supabase Dashboard → Authentication → Providers → Email → 'Email OTP Expiration'. Confirm it is well above 15 minutes (the default is 1 hour, which is fine). If you have tightened it to 15 minutes or less, lower RATE_WINDOW_MS in app/api/auth/forgot-password/route.ts so the cooldown window stays shorter than the link lifetime. Note Supabase's own security advisor flags LONG OTP expiry, so this is a genuine two-sided tradeoff, not a set-and-forget."
      blocks: none
    ...
    - id: apply-migration-029
      ...
      blocks: none
    - id: apply-migration-030
      title: "Apply migration 030_saved_designs_drop_permissive_public_policy.sql to prod (closes the share-link enumeration hole, issue #708) — APPLY BEFORE the DATA_BACKEND cutover"
      priority: high
      status: open
