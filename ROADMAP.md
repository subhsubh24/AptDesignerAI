...
- [ ] G4. **Auth failure-case hardening + tests**: lockout/backoff on repeated wrong
  ...
  **[Partial — Run 34 (PR #124): signup user-enumeration CLOSED — already-registered shows
  ...
  verification enumeration guards. Run 117: PASSWORD-RESET landed — the flow did not exist at all before (`resetPasswordForEmail` had zero hits repo-wide), so a locked-out user was permanently locked out; the new route is enumeration-safe by body, status AND latency (the outbound send is moved off the response path with `waitUntil` so only the registered branch's Resend round-trip cannot be timed), rate-limited 3/IP/15min behind Turnstile, and never claims mail was sent while the provider is in dry-run. REMAINING for G4: login lockout/backoff (owner-config, above) and email-verification link idempotency. Run 127: the "per-IP only" gap is now PARTLY closed — a per-RECIPIENT quota (3 sends per address per 15 min) caps the OUTBOUND side, so a distributed sender can no longer bomb one inbox from many hosts. It counts DELIVERED mail rather than attempts (every non-delivery path releases its claim), which is what stops it from silently suppressing the account owner's OWN recovery behind an empty inbox — the first design did exactly that and two reviewers rejected it. Only PARTLY closed, and the code says so: the bucket is per-serverless-instance in-memory, so the real ceiling is 3 x warm instances (PENDING_OPS `rate-limit-redis` closes it outright). Keep G4 unchecked: login lockout/backoff (owner-config, above) and email-verification link idempotency are both still untouched. Run 141 (PR #789, merged e491a6c): EMAIL-VERIFICATION LINK IDEMPOTENCY CLOSED — the password-reset confirmation link (the one email-based, token-redemption link this app has) is now double-click safe: a redemption marker scoped to the exact `token_hash` (`lib/auth/reset-link-idempotency.ts`, `redeemResetLink()`) distinguishes "I personally just redeemed this token" from "a session merely exists in this browser," closing a shared-machine bypass an earlier revision introduced (caught in review) where any live session would mark a dead/garbage link "ready." 11 tests cover the decision table and the actual wiring sequence (call order, which key is written/read, cross-token isolation). Two independent reviewer cycles plus a fresh cycle-3 confirmation. Keep G4 unchecked: login lockout/backoff (owner-config, above) is the sole remaining item.]**
